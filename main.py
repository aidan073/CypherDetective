...
from save_system import load_progress, save_progress, complete_level, is_level_unlocked, get_highest_unlocked_level
...
class Game:
    ...
    def execute_query(self):
        ...
        if not self.db:
            if self.db_error:
                ...
                self.error_message = "Database not connected. Please configure your Neo4j connection."
            return
    ...
    def render_menu(self):
        ...
        # Subtitle
        subtitle = FONT_MEDIUM.render("Solve the crime with Cypher queries", True, COLORS['text_dim'])
        subtitle_rect = subtitle.get_rect(center=(SCREEN_WIDTH // 2, 260))
        self.screen.blit(subtitle, subtitle_rect)
        
        # Database connection status
        if self.db:
            status_text = FONT_SMALL.render("✓ Database Connected", True, COLORS['success'])
        else:
            status_text = FONT_SMALL.render("✗ Database Not Connected", True, COLORS['error'])
        ...
        for i in range(0, SCREEN_WIDTH, 20):
            pygame.draw.line(self.screen, COLORS['darker_bg'], (i, 0), (i + 200, SCREEN_HEIGHT), 1)
    ...
    def render_level_select(self):
        ...
        for i in range(1, total_levels + 1):
            ...
            # Level text
            if unlocked:
                level_text = FONT_MEDIUM.render(f"Level {i}: {level.title}", True, COLORS['text_bright'])
            else:
                level_text = FONT_MEDIUM.render(f"Level {i}: Locked", True, COLORS['text_dim'])
            ...
            # Key hint
            if unlocked:
                key_text = FONT_TINY.render(f"Press {i} to play", True, COLORS['text_dim'])
                key_rect = key_text.get_rect(center=(button_rect.centerx, button_rect.centery + 30))
                self.screen.blit(key_text, key_rect)
        
        # Back instruction
        back_text = FONT_SMALL.render("Press ESC to return to menu", True, COLORS['text_dim'])
    ...
    def render_gameplay(self):
        ...
        # Level title
        title = FONT_MEDIUM.render(f"Level {self.current_level.level_num}: {self.current_level.title}", True, COLORS['accent'])
        ...
        # Wrap clue text
        clue_lines = self.wrap_text(self.current_level.lead, FONT_SMALL, clue_box.width - 20)
        ...
        if self.current_level.hint:
            ...
            hint_lines = self.wrap_text(self.current_level.hint, FONT_TINY, SCREEN_WIDTH - 100)
            y_offset = hint_y + 20
        ...
        query_lines = self.wrap_text(self.current_query, FONT_SMALL, input_box.width - 40)
        ...
        if self.cursor_visible and y_offset < input_box.bottom - 10:
            ...
            if query_lines:
                ...
                cursor_x += text_width
            pygame.draw.line(self.screen, COLORS['text_bright'], 
                           (cursor_x, y_offset - 20), (cursor_x, y_offset - 5), 2)
        
        # Error/Success message
        message_y = input_box.bottom + 20
        if self.error_message:
            error_lines = self.wrap_text(self.error_message, FONT_SMALL, SCREEN_WIDTH - 100)
            y_offset = message_y
        ...
        if self.success_message:
            success_lines = self.wrap_text(self.success_message, FONT_SMALL, SCREEN_WIDTH - 100)
            y_offset = message_y
        ...
        if self.query_result is not None:
            ...
            result_lines = self.wrap_text(result_text, FONT_TINY, results_box.width - 20)
    ...
    def render_result(self):
        ...
        if self.success_message:
            ...
            next_text = FONT_MEDIUM.render("Press ENTER to continue", True, COLORS['text'])
            ...
            retry_text = FONT_SMALL.render("Press ENTER to try again", True, COLORS['text'])
