...
level_1 = Level(
    ...
    hint="Look for suspects with the verified_alibi property set to false. Try: MATCH (s:Suspect) WHERE s.verified_alibi = false RETURN s.name AS suspect",
)
...
level_2 = Level(
    ...
    hint="Look for suspects with a WAS_AT relationship to anything but the victim's building. Try: MATCH (s:Suspect) WHERE NOT (s)-[:WAS_AT]->(:Location {name: \"Victim's Apartment Building\"}) RETURN s.name AS suspect",
)
...
level_3 = Level(
    ...
    hint='Look for suspects with a WORKS_AT relationship to the Grandview Hotel. Try: MATCH (s:Suspect)-[:WORKS_AT]->(h:Location {name: "Grandview Hotel"}) RETURN s.name AS suspect',
)
...
level_4 = Level(
    ...
    hint="Filter hotel employees by access_level property. Try: MATCH (s:Suspect) WHERE s.access_level >= 2 RETURN s.name AS suspect",
)
...
level_5 = Level(
    ...
    hint='Filter suspects by hair color and height. Try: MATCH (s:Suspect) WHERE s.hair = "brown" AND s.height >= 6.0 RETURN s.name AS suspect',
)
...
level_6 = Level(
    ...
    hint='Filter suspects by blood_type property. Try: MATCH (s:Suspect) WHERE s.blood_type = "O+" RETURN s.name AS suspect',
)
...
level_7 = Level(
    ...
    hint='Look for suspects with a CLOSE_FRIEND_OF relationship to John Doe. Try: MATCH (v:Victim {name: "John Doe"})<-[:CLOSE_FRIEND_OF]-(s:Suspect) RETURN s.name AS suspect',
)
...
level_8 = Level(
    ...
    hint="Sum deposit amounts using aggregation. Try: MATCH (s:Suspect)-[r:DEPOSITED_IN]->(:Bank) WITH s, SUM(r.amount) AS total WHERE total = 475500 RETURN s.name AS suspect",
)
